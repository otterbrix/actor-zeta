...
public:
    ...
    void prepare() {
        ...
        actor_0_ = actor_zeta::spawn<Actor>(resource(), scheduler_);
        actor_1_ = actor_zeta::spawn<Actor>(resource(), scheduler_);
    ...
    void send() {
        ...
        if (actor_0_ && scheduler_) {
            ...
            if (actor_zeta::send(actor_0_.get(), this->address(), &Actor::start)) {
